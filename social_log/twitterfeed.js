...
async function create_twit(client){
      ...
      if(!userids || userids.length == 0) return console.log(`NO USERIDS: ${userids}`.gray)
      //create a new TWIT
      var T = new Twit({
      ...
      console.log(String(userids.filter(i=> i.toLowerCase() != "impressgm").join(", ")).cyan)
      for(const user of userids){
        if(String(user).toLowerCase().includes("impressgm")) continue;
        console.log(String("TWITTER - " + user).cyan);
        gettwit(user);
        await delay(1500);
      }
      ...
      async function gettwit(user){
        try{
          await T.get('search/tweets', { q: `from:${user}`, count: 10 }, function(err, data, response) {
            try{
              console.log("TWITTER - DATA RECEIVED".cyan)
              ...
              if(!guildid || guildid == null || guildid == undefined || guildid.length != 18) return console.log("NO VALID GUILD ".cyan)
              try{
              ...
              if(twitsettings.latesttweet === tweet.id_str) return console.log("LATEST TWEET".cyan)
              ...
              return console.log(String("NOT RIGHT TWEET! :: " + url).cyan)
              //If retweet, do that
              ...
              //If sent tweet, do that
                ...
                client.channels.fetch(twitsettings.DISCORD_CHANNEL_ID).then(channel => {
                  channel.send(twitsettings.infomsg.replace("{Twittername}", TwitterName).replace("{url}", url)).then(msg=>{
                    console.log(`TWITTER NOTIFICATION SENT IN ${channel.name} for ${TwitterName} with ${url}`.green)
                    //set the new latest tweet
                    client.social_log.set(guildid, tweet.id_str, "twitter.latesttweet")
                  }).catch(e=>{
